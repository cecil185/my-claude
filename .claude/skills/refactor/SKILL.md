...
Work through the full smell checklist in [reference.md](reference.md) — size, naming,
duplication, responsibility, conditionals, data, comments, testability, debuggability,
and async. For each smell found, record: location (`file:line`), a one-line description,
and severity (Major = fix now, Minor = fix if small, Defer = ticket it).
