# Test Gap Scan Reference

## Contents

- [Gap definitions](#gap-definitions)
- [Gap B anti-patterns](#gap-b-anti-patterns)
- [Coverage output sections](#coverage-output-sections)
- [Interpreting coverage output](#interpreting-coverage-output)
- [Gap B assessment checks](#gap-b-assessment-checks)
- [High-value behavioural scenarios](#high-value-behavioural-scenarios)
- [Report template](#report-template)
- [Linear ticket templates](#linear-ticket-templates)
...
Assertions are tautological, only the happy path is covered, or mocking is so
aggressive the real code is never called.
...
## Coverage output sections

`parse_coverage.py` emits four sections:
...
### Verifying Gap A with fff search
...
Batch multiple function names into one `mcp__fff__multi_grep` call per module to keep it fast.
...
## Gap B assessment checks

For each test file, answer all five:
...
## High-value behavioural scenarios

Beyond file-level gaps, check whether a test covers each of these. If not, it's a
