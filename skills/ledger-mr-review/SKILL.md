...
description: Conducts an adversarial Merge Request / Pull Request code review referencing MR review standards, and appends the structured adversarial review to <repo>/code-review.md as an additive-only debate ledger.
---

# Ledger MR Review Skill (`ledger-mr-review`)

## Purpose
The `ledger-mr-review` skill performs a strict, adversarial ("red-team") code review on Git changes, Merge Requests (MRs), or Pull Requests (PRs). Rather than producing a basic summary, it scrutinizes changes from a critical perspective—assuming bugs, race conditions, edge-case failures, unhandled nulls, and performance regressions exist until proven otherwise.

All reviews generated by this skill are recorded into an **additive-only ledger file** (`<repo>/code-review.md`) to facilitate multi-model asynchronous debate.

---

## Key Principles & Execution Workflow

### 1. Adversarial Mindset (Referencing Standard MR Review Principles)
...
**Model / Role**: Primary Reviewer (Adversarial)  
...
## Usage Guide
1. Inspect git diffs, commits, or working tree changes in the target repository.
2. Formulate an adversarial critique identifying specific failure modes and vulnerabilities.
3. Check if `<repo>/code-review.md` exists. Read its contents.
4. Append the formatted adversarial review to `<repo>/code-review.md`.
